...
def load_chosen_map(filepath="chosen_map.txt") -> dict:
    """
    Loads a mapping from a file with lowercase keys only.
    Example line in file: shi=し
    """
    chosen_map = {}
    try:
...
def replace_text(text: str, mapping: dict) -> str:
    """
    Greedy replace using provided mapping (sorted by longest key first).
    """
    result = []
    i = 0
...
def convert_from_chosen_map(text: str, chosen_map: dict) -> list[str]:
    """
    Adds 2 entries from chosen_map per paragraph, cumulatively.
    Skips signs not found in the text.
    """
    paragraphs = []
    cumulative = {}
...
def main_chosen():
    ...
    #Convert input text to lowercase for consistent mapping
    with open(input_file, "r", encoding="utf-8") as f:
        text = f.read().lower()

    chosen_map = load_chosen_map(chosen_file)
